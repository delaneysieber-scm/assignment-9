...
# Your code here
product_units_total_sales_info= sales_df[['Product', 'Units', 'Total_Sales']]
print(product_units_total_sales_info)
...
# Your code here
high_volume_sales_condition= sales_df['Units'] >20
high_volume_sales= sales_df[high_volume_sales_condition]

# 3.3 Select sales of the 'PhoneX' product that were on promotion
# REQUIRED: Store result in variable 'phonex_promo'
# Your code here
mask= (sales_df['Product']=='PhoneX') & (sales_df['Promotion'] == 'Yes')
phonex_promo= sales_df[mask]
print(phonex_promo)
...
# Your code here
best_product_row=sales_df.loc[sales_df['Total_Sales'].idxmax()]
...
# Your code here
grouped_regions=sales_df.groupby('Region')['Total_Sales'].sum()
sales_by_region=grouped_regions.sort_values(ascending=False)
...
# Your code here
missing_percentages=((missing_counts / len(sales_df)) * 100).round(2)
...
total_revenue_all=sales_df['Total_Sales'].sum()
product_revenue_analysis= pd.DataFrame({
    'total_revenue':product_revenue,
    'percentage':(product_revenue/total_revenue_all)*100
})
...
print("-Improve categories that are not performaing well by reviewing pricing.")
print("-Lock in on regions that are performing the best to maximize potential revenue.")
print("-Optimize promotions on products like PhoneX to improve sales.")